...
informacion_pokemon_t *pokemon_cargar_archivo(const char *path)
{
	...
	informacion =
		(informacion_pokemon_t *)malloc(sizeof(informacion_pokemon_t));
	...
	while (!feof(archivo)) {
		if (contar_separadores(linea) == 1) {
			...
			for (i = 0; i < MAX_ATAQUES; i++) {
				...
				sscanf(linea, "%[^;];%c;%u",
				       poke.info_ataque[i].nombre, &letra,
				       &(poke.info_ataque[i].poder));
		...
		nuevo = (pokemon_t *)realloc(
			pokemon,
			sizeof(pokemon_t) *
				(long unsigned int)(informacion->cantidad + 1));
		if (nuevo == NULL) {
			fclose(archivo);
			return NULL;
		}
					   ...
					   const char *nombre)
{
	...
	if (pokemon == NULL || nombre == NULL) {
		...
		while (resultado == NULL && i < MAX_ATAQUES) {
			if (strcmp(pokemon->info_ataque[i].nombre,
				   (char *)nombre) == 0) {
